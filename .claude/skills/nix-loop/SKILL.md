---
name: nix-loop
description: Drive a feature's Próba (nixosTest) to green — build it, read the failure, fix the feature or its test, repeat. The feedback engine reused by /install-feature and /update-feature. Use when a feature's Próba is red, or to verify a feature works in a VM.
---
...
1. **Read the Dziennik first.** Open `modules/.../<f>/notes.md` (if it exists) so
   you don't re-derive problems already solved. See [[project_proba_harness]].
2. **Build the Próba:** `nix build .#checks.x86_64-linux.feature-<f> -L`
...
4. **Red?** Diagnose, then fix and rebuild:
   - *Eval/build error* (Nix): usually a missing `requires`, an undeclared option,
     ...
     in the Próba (`lib.mkForce` blank `sops.secrets/templates/age.sshKeyPaths`,
   ...
   - Edit the feature file and/or its `probaTests.<f>.testScript` autonomously
...
6. **GUI Próby** may be flaky — retry once before declaring failure.
...
- `feature-coverage` (`nix build .#checks.x86_64-linux.feature-coverage`) asserts
  every feature with `featureMeta` also has a Próba and vice-versa.
