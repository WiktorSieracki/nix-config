...
description: Add a new standalone feature (package/program/service) to the nix-config — scaffold the module + featureMeta + Próba + Dziennik, drive it green in a VM, then wire it into a host. Use when the user wants to install/add software or a service to their NixOS config.
---
...
3. **Dziennik:** create `modules/features/<category>/<name>/notes.md` with a header.
4. **Track it:** `git add` the new folder (flakes ignore untracked files).
5. **Green it:** run **/nix-loop** for `<name>` until the Próba passes.
